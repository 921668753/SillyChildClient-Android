...
//                Glide.with(activity)
//                        //      .asBitmap()//配置上下文
//                        .load(emMessage.getStringAttribute("avatar", null))//设置图片路径(fix #8,文件名包含%符号 无法识别和显示)
//                        .error(R.drawable.ease_default_avatar)           //设置错误图片
//                        .placeholder(R.drawable.ease_default_avatar)     //设置占位图片
//                        .fallback(R.drawable.ease_default_avatar)//当url为空时，显示图片
...
//                Glide.with(activity)
//                        //      .asBitmap()//配置上下文
//                        .load(emMessage.getStringAttribute("to_avatar", null))//设置图片路径(fix #8,文件名包含%符号 无法识别和显示)
//                        .error(R.drawable.ease_default_avatar)           //设置错误图片
//                        .placeholder(R.drawable.ease_default_avatar)     //设置占位图片
//                        .fallback(R.drawable.ease_default_avatar)//当url为空时，显示图片
//                        .diskCacheStrategy(DiskCacheStrategy.ALL)//缓存全尺寸
...
//        }
        ...
        if (userAvatarView != null) {
            ...
            if (message.direct() == Direct.SEND) {
                 ...
                 * by bugzhu
                 */
                Glide.with(context).load(message.getStringAttribute("avatar", EMClient.getInstance().getCurrentUser())).diskCacheStrategy(DiskCacheStrategy.ALL).placeholder(R.drawable.ease_default_avatar).into(userAvatarView);
//                    EaseUserUtils.setUserAvatar(context, EMClient.getInstance().getCurrentUser(), userAvatarView);
            } else {
                /**
                 * by bugzhu
                 */
                Glide.with(context).load(message.getStringAttribute("avatar", message.getFrom())).diskCacheStrategy(DiskCacheStrategy.ALL).placeholder(R.drawable.ease_default_avatar).into(userAvatarView);
                usernickView.setText(message.getStringAttribute("nickname", message.getFrom()));
//                EaseUserUtils.setUserAvatar(context, message.getFrom(), userAvatarView);
//                EaseUserUtils.setUserNick(message.getFrom(), usernickView);
            }
        }
