    ...
    private void setUpBaseView() {
        ...
        if (userInfoProvider != null) {
            ...
            if (message.direct() == Message.Direct.RECEIVE) {
                ...
                //   UserUtil.setAgentNickAndAvatar(context, message, userAvatarView, usernickView);
                // UserUtil.setCurrentUserNickAndAvatar(context,message, userAvatarView, usernickView);
            }
        }
...
//                    Glide.with(activity)
//                            //      .asBitmap()//配置上下文
//                            .load(message.getStringAttribute("avatar", null))//设置图片路径(fix #8,文件名包含%符号 无法识别和显示)
//                            .error(R.drawable.hd_default_avatar)           //设置错误图片
//                    //        .placeholder(R.drawable.hd_default_avatar)     //设置占位图片
//                            .fallback(R.drawable.hd_default_avatar)//当url为空时，显示图片
...
//                            .into(userAvatarView);
                    ...
                    // ((MessageAdapter) adapter).getItem(adapter.getCount() - 1);
                    userAvatarView.setBackground(null);
                    Glide.with(activity)
                            //      .asBitmap()//配置上下文
                            .load(message.getStringAttribute("avatar", null))//设置图片路径(fix #8,文件名包含%符号 无法识别和显示)
