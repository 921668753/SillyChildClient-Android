...
//        titleView.setLayoutParams(new RelativeLayout.LayoutParams(LayoutParams.MATCH_PARENT, dip2px(48)));
        ...
        v_dividercolor=(View)titleView.findViewById(R.id.v_dividercolor);
    ...
    public class WebViewClient extends android.webkit.WebViewClient {
        ...
        private WebViewClientCallback mWebViewClientHandle;

        public WebViewClient(WebViewClientCallback mWebViewClientHandle){
            this.mWebViewClientHandle=mWebViewClientHandle;
        }
...
//                titleText.setText(view.getTitle());
        ...
        public boolean shouldOverrideUrlLoading(WebView view, String url) {
            if (mWebViewClientHandle!=null){
                if (url.indexOf("tel:") == 0) {// 页面上有数字会导致连接电话
                    mWebViewClientHandle.onUrlString(0, url);
                    return true;
                }else if (url.indexOf("coordinates:")==0){
                    mWebViewClientHandle.onUrlString(1, Uri.decode(url.replace("coordinates:","")));
                    return true;
                }else if (url.indexOf("landlord:")==0){
                    mWebViewClientHandle.onUrlString(2, url.replace("landlord:",""));
                    return true;
                }else if (url.indexOf("seeevaluation:")==0){
                    mWebViewClientHandle.onUrlString(3, null);
                    return true;
                }else if (url.indexOf("houses:")==0){
                    mWebViewClientHandle.onUrlString(4, url.replace("houses:",""));
                    return true;
                }
            }
            //如果没有自己要做的操作就显示加载网页
...
//        }
    ...
    public void setWebViewClientCallBack(WebViewClientCallback callBack) {
    ...
    public void setDividerColor(int colorres){
        try {
            v_dividercolor.setVisibility(View.VISIBLE);
            v_dividercolor.setBackgroundResource(colorres);
        }catch (Exception e){
    ...
    public void addJavaScriptMethod(Object object, String methodname){
        if (webView!=null){
            webView.addJavascriptInterface(object,methodname);
        }
    }
}