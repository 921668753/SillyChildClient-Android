package com.yinglan.scm.utils.easeim;
...
//import com.yinglan.scm.homepage.customerservice.OverleafActivity;
import com.yinglan.scm.main.MainActivity;
//import com.yinglan.scm.message.ChatMessageActivity;
import com.yinglan.scm.utils.easeim.db.DemoDBManager;
import com.yinglan.scm.utils.easeim.db.InviteMessgeDao;
import com.yinglan.scm.utils.easeim.db.UserDao;
import com.yinglan.scm.utils.easeim.domain.InviteMessage;
import com.yinglan.scm.utils.easeim.domain.InviteMessage.InviteMessageStatus;
import com.yinglan.scm.utils.easeim.domain.RobotUser;
import com.yinglan.scm.utils.easeim.parse.UserProfileManager;
import com.yinglan.scm.utils.easeim.utils.PreferenceManager;
...
//    private CallReceiver callReceiver;
    ...
    public void init(Context context) {
        ...
        if (EaseUI.getInstance().init(context, options)) {
            ...
            if (minBitRate != -1) {
                // EMClient.getInstance().callManager().getOptions().setMinVideoKbps(minBitRate);
            }

            // max video kbps
            int maxBitRate = PreferenceManager.getInstance().getCallMaxVideoKbps();
            if (maxBitRate != -1) {
                //  EMClient.getInstance().callManager().getOptions().setMaxVideoKbps(maxBitRate);
            }

            // max frame rate
            int maxFrameRate = PreferenceManager.getInstance().getCallMaxFrameRate();
            if (maxFrameRate != -1) {
                //  EMClient.getInstance().callManager().getOptions().setMaxVideoFrameRate(maxFrameRate);
            }

            // audio sample rate
            int audioSampleRate = PreferenceManager.getInstance().getCallAudioSampleRate();
            if (audioSampleRate != -1) {
                //  EMClient.getInstance().callManager().getOptions().setAudioSampleRate(audioSampleRate);
            }
            ...
            if (wh.length == 2) {
                try {
                    //   EMClient.getInstance().callManager()..setVideoResolution(new Integer(wh[0]).intValue(), new Integer(wh[1]).intValue());
                } catch (Exception e) {
                    e.printStackTrace();
                }
            }
            ...
            //  EMClient.getInstance().callManager().getOptions().enableFixedVideoResolution(enableFixSampleRate);

            // Offline call push
            //EMClient.getInstance().callManager().getOptions().setIsSendPushIfOffline(getModel().isPushCall());
    ...
    private void setCallOptions() {
        ...
        if (wh.length == 2) {
            try {
                //   EMClient.getInstance().callManager().getOptions().setVideoResolution(new Integer(wh[0]).intValue(), new Integer(wh[1]).intValue());
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
...
//                }
        ...
        easeUI.getNotifier().setNotificationInfoProvider(new EaseNotificationInfoProvider() {
            ...
            public Intent getLaunchIntent(EMMessage message) {
                ...
                if (message.conversationId().equals(BuildConfig.HUANXIN_IM)) {
                  //  Intent intent = new Intent(appContext, OverleafActivity.class);
                   // return intent;
                }
               // Intent intent = new Intent(appContext, ChatMessageActivity.class);
