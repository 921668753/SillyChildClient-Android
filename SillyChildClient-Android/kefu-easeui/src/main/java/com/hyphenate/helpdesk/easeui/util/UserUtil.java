            ...
            if (agentInfo != null) {
                if (!TextUtils.isEmpty(agentInfo.getAvatar())) {
                    ...
                    if (!TextUtils.isEmpty(strUrl)) {
                        ...
                        //正常的string路径
                        Glide.with(context).load(strUrl).diskCacheStrategy(DiskCacheStrategy.ALL).placeholder(R.drawable.hd_default_avatar).transform(new GlideCircleTransform(context)).into(userAvatarView);
                    }
                }
            }
